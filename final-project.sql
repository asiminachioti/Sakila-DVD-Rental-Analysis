...
-- 5. Find the number of films per month, rented to a customer by the staff member Mike Hillyer in 2005.
...
-- 6. List each film and its actor count.
...
-- 7: Find the films title, rental date, rental rate and replacement cost that are rented by inactive customers that live in the country “Virgin Islands, U.S.”.
...
-- 8: Find the number of unique customers that returned a film in 9 days or more
...
-- 9. Use subqueries to display all actors who appear in the film Alone Trip.
...
-- 10: You want to run an email marketing campaign in Canada, for which you will need the names and email addresses of all Canadian customers. 
...
-- 12: Find the total length of the unique films rented by customers whose first name starts with G and the returned date happened in the first 6 months of 2005.
...
-- 13: Total number of rentals and amount per store, city, and film category by month in 2005
...
-- 14: Find the most rented movie in each category.
...
-- 15: Write a query to show daily, weekly, and monthly active users. Make the assumption that the current date is 2005-08-30. 
...
-- 16: Calculate the cumulative Payment Amount per Customer and Payment Date.
...
-- 17: Rank top 100 customers based on their total revenue. (Make use of window functions)
...
-- 18: Find the most popular actor by each Category, based on the number of times their movies are rented. (Make use of CTEs)
...
-- 19: What are the top 3 most rented film categories per store in 2005, based on rental count, revenue, and unique customers?
...
-- 20. List the top five genres in gross revenue in descending order. (Hint: you may need to use the following tables: category, film_category, inventory, payment, and rental.)
...
-- 21: Find the top 10 customers with the most late returns (films returned after rental duration).
